...
Author: Midea Group
*/
// 构造函数。delete_param: 删除准则参数,balance_param: 平衡准则参数,box_length: 立方体边长,用于下采样
...
// 初始化函数。delete_param: 删除准则参数,balance_param: 平衡准则参数,box_length: 立方体边长,用于下采样
template <typename PointType>
...
int KD_TREE<PointType>::validnum(){
    ...
    if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node){
        ...
        if (!pthread_mutex_trylock(&working_flag_mutex)){
            ...
            pthread_mutex_unlock(&working_flag_mutex);
            return s;
        } else {  // 加锁失败,返回-1表示正在修改树
            return -1;
        }
    }
}

// 获取根节点的平衡参数alpha_bal和删除参数alpha_del
template <typename PointType>
void KD_TREE<PointType>::root_alpha(float &alpha_bal, float &alpha_del){
    if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node){  // 当前没有重建或者重建指针不指向根节点
...
void KD_TREE<PointType>::multi_thread_rebuild(){
    ...
    while (!terminated){
        ...
        if (Rebuild_Ptr != nullptr ){   // 若有需要重建的子树                  
            /* Traverse and copy */
            if (!Rebuild_Logger.empty()){
                printf("\n\n\n\n\n\n\n\n\n\n\n ERROR!!! \n\n\n\n\n\n\n\n\n");
            }
            rebuild_flag = true;
            if (*Rebuild_Ptr == Root_Node) {  // 若重建指针指向根节点
                Treesize_tmp = Root_Node->TreeSize;  // 备份树大小
                Validnum_tmp = Root_Node->TreeSize - Root_Node->invalid_point_num;  // 备份有效节点数
                alpha_bal_tmp = Root_Node->alpha_bal;
                alpha_del_tmp = Root_Node->alpha_del;
            }
            KD_TREE_NODE * old_root_node = (*Rebuild_Ptr);  // 保存旧的根节点指针                            
...
// 获取ikd树删除的点。removed_points为删除的点云
template <typename PointType>
...
// 区域删除。root为当前子树根指针,boxpoint为删除区域,allow_rebuild为是否允许重建标志,is_downsample为是否下采样标志,返回删除的点数
template <typename PointType>
...
// 单点添加。root为当前子树根指针,point为添加的点,allow_rebuild为是否允许重建标志,father_axis为父节点的划分轴
template <typename PointType>
void KD_TREE<PointType>::Add_by_point(KD_TREE_NODE ** root, PointType point, bool allow_rebuild, int father_axis){     
    ...
    if (Rebuild_Ptr != nullptr && *Rebuild_Ptr == *root && (*root)->TreeSize < Multi_Thread_Rebuild_Point_Num) 
      Rebuild_Ptr = nullptr;  // 如果当前节点为重建指针,且子树尺寸小于阈值,则将重建指针置空
    bool need_rebuild = allow_rebuild & Criterion_Check((*root));
    if (need_rebuild) Rebuild(root); 
    if ((*root) != nullptr) (*root)->working_flag = false;   
    return;
...
// k近邻搜索。root为当前k-d树根节点,k_nearest为查找的近邻数,point为查询点,q为优先队列,存储查找结果,max_dist为查找的最大距离
template <typename PointType>
...
void KD_TREE<PointType>::Update(KD_TREE_NODE * root){
    ...
    // 如果是根节点且树大小大于3,计算alpha_del和alpha_bal
...
template <typename PointType>
size_t KD_TREE<PointType>::memory_usage() const {
    return memory_usage(Root_Node);
}

template <typename PointType>
size_t KD_TREE<PointType>::memory_usage(KD_TREE_NODE* node) const {
    if (node == nullptr) {
        return 0;
    }

    size_t node_size = sizeof(KD_TREE_NODE);
    size_t left_size = memory_usage(node->left_son_ptr);
    size_t right_size = memory_usage(node->right_son_ptr);
