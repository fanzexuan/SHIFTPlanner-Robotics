...
// 常量定义
#define EPSS 1e-6                           // 浮点数比较的误差阈值
#define Minimal_Unbalanced_Tree_Size 10     // 最小不平衡树的大小
#define Multi_Thread_Rebuild_Point_Num 1500 // 触发多线程重建的点数阈值
#define DOWNSAMPLE_SWITCH true              // 是否开启下采样
#define ForceRebuildPercentage 0.2          // 强制重建的点百分比
#define Q_LEN 1000000                       // 操作队列的长度

using namespace std;

// ikd-Tree自定义的点类型，支持2D和3D数据
...
public:
    ...
    // k最近邻搜索。point: 搜索的中心点,k_nearest: 搜索的邻居数量,Nearest_Points: 搜索结果,Point_Distance: 搜索结果对应的距离,max_dist: 搜索的最大半径,默认为无穷大
    ...
    // 矩形区域搜索。Box_of_Point: 搜索的矩形区域,Storage: 搜索结果 
    ...
    // 增量式添加点云到ikd树。PointToAdd: 待添加的点云,downsample_on: 是否对添加的点云下采样,返回值为点数    
    ...
    int max_queue_size = 0; // Rebuild_Logger队列的最大长度

    public:
    size_t memory_usage() const;

    private:
    size_t memory_usage(KD_TREE_NODE* node) const;
